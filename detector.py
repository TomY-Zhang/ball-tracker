...
class Detector:
    ...
    # draws a bounding box into an image
    def draw_bbox(self, image, threshold=0.5):
        # default color of OpenCV is BGR
        ...
        balls = [i for i in above_thresh.numpy() if class_ids[i] == SPORTS_BALL_ID]
        ...
        if len(bboxs) != 0:
            for i in balls:
                ...
                # unpack bbox coords and scale to image dimensions
                ymin, xmin, ymax, xmax = bbox
                xmin, xmax = int(xmin * imgW), int(xmax * imgW)
                ymin, ymax = int(ymin * imgH), int(ymax * imgH)

                # draw a rectangle with the bbox coords
                cv2.rectangle(img=image, pt1=(xmin, ymin), pt2=(xmax, ymax), color=(0, 255, 0), thickness=1)

                # add label text for each class
                ball_confidence = round(100 * class_scores[i])
                display_text = f"Sports ball: {ball_confidence}"
                cv2.putText(image, display_text, (xmin, ymin - 15), cv2.FONT_HERSHEY_PLAIN, 1, (0, 255, 0), 2)

        return image


    # capture and detect objects in video
    def detect_video(self, video_path, threshold=0.5):
        cap = cv2.VideoCapture(video_path)
        if not cap.isOpened():
            raise FileNotFoundError(f"Error opening file '{video_path}'")
        
        filename = "ball_detect.mp4"
        ...
        while success:
            # detect balls and draw bounding boxes on each frame of the video
            bbox_image = self.draw_bbox(frame, threshold)
            video.write(bbox_image)
